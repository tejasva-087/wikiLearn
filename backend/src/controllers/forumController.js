...
exports.createPost = catchAsync(async (req, res, next) => {
  ...
  // Format post with user interaction data
  const formattedPost = await formatPostWithUserInteraction(newPost, req.user.id);
...
exports.updatePost = catchAsync(async (req, res, next) => {
  ...
  // Format post with user interaction data
  const formattedPost = await formatPostWithUserInteraction(updatedPost, req.user.id);
...
exports.getSavedPosts = catchAsync(async (req, res, next) => {
  ...
  // Find the posts
  const posts = await ForumPost.find({ _id: { $in: postIds } })
    .sort({ createdAt: -1 });
