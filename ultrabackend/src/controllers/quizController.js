...
const runQuizGenerator = require("../utils/quizGenerator");
const AppError = require("../utils/AppError");

exports.fivePiller = catchAsync(async (req, res, next) => {
  const data = `The fundamental principles of Wikipedia may be summarized in five "pillars":
  ...
  try {
    const questions = await runQuizGenerator(data);
    res.status(200).json({
      status: "success",
      data: {
        answer: questions,
    ...
    return next(new AppError("There was an error with the AI model", 404));
  }
});
